...
# Fit Naive Bayes model using training data
nbModel <- naiveBayes(Churn ~ ., data = trainData)

# Predict on test data
predictions <- predict(nbModel, testData[, -20])

# Evaluate model performance
library(caret)
confusionMatrix(predictions, as.factor(testData$Churn))
...
# Set the predictor variables and response variable
predictor_vars <- c("tenure", "MonthlyCharges", "TotalCharges")
response_var <- "Churn"

# Train the k-NN model
knn_model <- knn(train = train_data[, predictor_vars], 
                 test = test_data[, predictor_vars], 
                 cl = train_data[, response_var], 
                 k = 5)

# Evaluate the model performance
confusion_matrix <- table(knn_model, test_data[, response_var])
accuracy <- sum(diag(confusion_matrix)) / sum(confusion_matrix)
print(paste("Accuracy:", accuracy))
# Confusion matrix with sensitivity, specificity, etc.,
confusionMatrix(knn_model, test_data$Churn)



############## 7. Under sampling on decision tree
...
############## 7. Over sampling on decision tree
...
############## 9. SMOTTEEN method on decision tree
...
############## 10. Tuning and staistical analysis
